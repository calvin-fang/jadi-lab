function [] = spontaneous_lfp_state_classifier_parallel(dur,layer,fineScale,scramble,iter,forest_size,trnMBPID,tstMBPID,sampleSz)
%{
...
scramble: 1 for scrambled, 0 for not scrambled
iter: current iteration
forest_size:size of forest
trnMBPID,tstMBPID,sampleSz: loading pre created indices
...
outfnPre = ['state_classifier_layer_' num2str(layer) '_duration_' num2str(dur) 'ms_' scale '_iteration_' num2str(iter) '_output_MBP_' num2str(dur)];

if scramble
    outfnPre = ['scrambled_' outfnPre];
end

outfn = [outFile outfnPre '.mat'];
...
lockfn = [outFile outfnPre '_lock.mat'];
...
EX=transpose(MBP_ALL(layerInd,trnMBPID(iter,:))); %training data
LABEL=transpose(MBP_ALL(16,trnMBPID(iter,:))); %training labels
TEST=transpose(MBP_ALL(layerInd,tstMBPID(iter,:))); %test data
LABELhat=transpose((MBP_ALL(16,tstMBPID(iter,:)))); %test labels
...
%save(outfn,'bagger','layer', 'sampleSz', 'tstSmplSz', 'err', 'oobErr');
save(outfn,'bagger','layer', 'err', 'oobErr');
